...
describe TwitterClient::LinksExtractor, :vcr => true do
  describe "extract" do
    ...
    it "returns nils if no links found in tweet" do
      Twitter::Tweet.any_instance.stubs(:text).returns("foobar")
      links = TwitterClient::LinksExtractor.new.extract(username: "vdaubry", since: DateTime.parse("20/06/2015"))
      links.should == []
    end
  end
end